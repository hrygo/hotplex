...
def tests():
    write('internal/worker/opencodeserver/deep_audit_992_test.go', r'''
    ...
    for _, status := range []int{200, 204, 404, 405, 500} {
        t.Run(strconv.Itoa(status), func(t *testing.T) {
            ...
            if status == 200 || status == 204 {
                ...
                if status == 404 || status == 405 { require.ErrorIs(t,err,worker.ErrNotImplemented) }
            }
...
def fixes():
    replace('internal/worker/opencodeserver/worker.go',
        '\tif resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusNoContent && resp.StatusCode != http.StatusNotFound {',
        '''\t// A missing/unsupported endpoint is not evidence that native history was
    // cleared. Preserve local generation and connection state on this path.
