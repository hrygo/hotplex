...
def tests():
    write('internal/worker/codexcli/deep_audit_1003_test.go',r'''
package codexcli
...
func (s *deep1003ResponseWriter) Write(p []byte)(int,error){
    if s.output!=nil {if _,err:=s.output.Write(p);err!=nil{return 0,err}}
    ...
    if frame.Method=="turn/interrupt" && frame.ID!=0 && !s.drop {
        ...
        if s.reject {response=fmt.Sprintf(`{"id":%d,"error":{"code":-32602,"message":"no matching turn"}}`,frame.ID)}
...
func TestDeepAudit1003StopRequiresAcknowledgedRequest(t *testing.T){
    ...
    for _,reject:=range []bool{false,true}{
        name:="ack";if reject{name="rejection"}
        t.Run(name,func(t *testing.T){
            t.Parallel()
            m:=auditRPCManager();m.refs=2
            sink:=&deep1003ResponseWriter{manager:m,reject:reject};m.stdin=sink
            ...
            if reject {require.Error(t,err);require.False(t,w.IsStopped())} else {require.NoError(t,err);require.True(t,w.IsStopped())}
            require.Equal(t,2,m.refs)
            require.True(t,m.IsRunning())
            pending:=0;m.pending.Range(func(_, _ any)bool{pending++;return true});require.Zero(t,pending)
        })
    }
}
...
func TestDeepAudit1003MissingStopAckHonorsCallerBudget(t *testing.T){
    ...
    synctest.Test(t,func(t *testing.T){
        m:=auditRPCManager();sink:=&deep1003ResponseWriter{manager:m,drop:true};m.stdin=sink
        w:=&AppServerWorker{BaseWorker:base.NewBaseWorker(nil,nil),manager:m,threadID:"thread",turnID:"turn"}
...
func TestDeepAudit1003PreCancelledStopWritesNothing(t *testing.T){
    t.Parallel()
    m:=auditRPCManager();sink:=&deep1003ResponseWriter{manager:m};m.stdin=sink
    w:=&AppServerWorker{BaseWorker:base.NewBaseWorker(nil,nil),manager:m,threadID:"thread",turnID:"turn"}
...
def fixes():
    ...
    replace(path,'\terr := m.Notify(ctx, "turn/interrupt", map[string]any{','\t_, err := m.Call(ctx, "turn/interrupt", map[string]any{')
    replace(path,'// InterruptTurn interrupts the running turn in the specified thread.', '''// InterruptTurn requests interruption and waits for the server's bounded ACK.
// The ACK confirms acceptance only; turn/completed remains the terminal event.''')
    ...
    first='func TestAppServerWorker_StopThenTerminatePreservesSiblingWrapper(t *testing.T) {'
    start=text.index(first);end=text.index('\nfunc ',start+len(first))
    section=text[start:end]
    old='''\tmgr.stdin = struct {
\t\tio.Writer
\t\tio.Closer
\t}{
\t\tWriter: &output,
...
\t}'''
    ...
    section=section.replace(old,'\tmgr.stdin = &deep1003ResponseWriter{manager: mgr, output: &output}')
    text=text[:start]+section+text[end:]
    first='func TestClearWithActiveTurn(t *testing.T) {'
    start=text.index(first);end=text.index('\nfunc ',start+len(first))
    section=text[start:end]
    # Preserve the pipe and its cleanup, but make the fake peer return a real
    # RPC response instead of silently consuming the interrupt frame.
    old='\t\t_, _ = io.Copy(io.Discard, r)'
    new='''        decoder:=json.NewDecoder(r)
        for {
            var frame JSONRPCRequest
            if err:=decoder.Decode(&frame);err!=nil{return}
            if frame.Method=="turn/interrupt" && frame.ID!=0 {
                mgr.dispatchFrame([]byte(fmt.Sprintf(`{"id":%d,"result":{}}`,frame.ID)))
            }
        }'''
    assert section.count(old)==1
    section=section.replace(old,new).replace("// InterruptTurn's Notify needs a stdin writer. Provide a fake pipe.","// InterruptTurn requires a request/response peer. Preserve its real pipe.")
    text=text[:start]+section+text[end:]
